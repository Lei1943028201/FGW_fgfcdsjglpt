import Vue from "vue";
import TheHeader from './common/TheHeader'
import TheFooter from './common/TheFooter'
import CcTitle from './common/CcTitle'
import CcTab from './common/CcTab'
import CcTabText from './common/CcTabText'
import CcTop from './common/CcTop'
import CcEcharts from './common/CcEcharts'
import CcCard from './common/CcCard'

Vue.component('TheHeader', TheHeader)
Vue.component('TheFooter', TheFooter)
Vue.component('CcTitle', CcTitle)
Vue.component('CcTab', CcTab)
Vue.component('CcTabText', CcTabText)
Vue.component('CcTop', CcTop)
Vue.component('CcEcharts', CcEcharts)
Vue.component('CcCard', CcCard)
